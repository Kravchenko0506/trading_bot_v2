...
class GridConfig(StrategyConfig):
    ...
    rebalance_threshold: Decimal = Decimal('0.05')  # 5% price move to rebalance
    
    # Market condition filters
    volatility_threshold: Decimal = Decimal('0.02')  # Min 2% volatility for activation
    trend_filter_period: int = 50  # Period for trend detection
    max_trend_strength: Decimal = Decimal('0.03')  # Max 3% trend to stay active
...
class GridTradingStrategy(BaseStrategy):
    ...
    def _update_market_analysis(self, current_price: Decimal):
        ...
        if len(self.price_history) >= 20:
            recent_prices = np.array([float(p) for p in self.price_history[-20:]])
    ...
    def _check_market_conditions(self, current_price: Decimal) -> Dict[str, Any]:
        ...
        if self.volatility_buffer:
            avg_volatility = sum(self.volatility_buffer) / len(self.volatility_buffer)
    ...
    def _check_grid_triggers(self, current_price: Decimal) -> TradingSignal:
        ...
        for i, level in enumerate(self.grid_levels):
            ...
            trigger_threshold = level.price * Decimal('0.001')  # 0.1% threshold
        ...
        return TradingSignal(
            ...
            confidence=0.5,
            indicators={'active_grid_levels': len([l for l in self.grid_levels if not l.is_filled])}
        )
    ...
    def _needs_rebalancing(self, current_price: Decimal) -> bool:
        ...
        center_deviation = abs(current_price - self.grid_center) / self.grid_center
    ...
    def _rebalance_grid(self, current_price: Decimal):
        ...
        self.logger.info(f"Rebalancing grid around new center: {current_price:.6f}")
